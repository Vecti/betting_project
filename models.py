...
class Models():
    ...
    Currently empty,
    in future with abstract methods like evaluate/score/predict etc
    """
...
class PoissonModel(Models):
    """
    Basic model which calculates attack/def ratio for given clubs
    and calculates probabilities for given scenario using
    ...
    def poisson_formula(self, goals_expected, n):
        """
        simplified version
        for given team we provide number of goals expected from home_team
        ...
        mass_dictionary = {}
        for i in range(n):
            mass_dictionary[i] = poisson.pmf(i, goals_expected)

        mass_dictionary['over ' + str(n-1)] = (1 - sum(mass_dictionary.values()))
